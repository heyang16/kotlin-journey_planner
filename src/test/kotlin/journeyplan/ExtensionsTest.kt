...
class ExtensionsTest {
  ...
  fun `can find multiple routes between stations`() {
    ...
    val routes1 = map.routesFrom(southKensington, victoria)
    val routes2 = map.routesFrom(southKensington, oxfordCircus)
    val routes3 = map.routesFrom(southKensington, southKensington)
    assertEquals(2, routes1.size)
    assertEquals(2, routes2.size)
    assertEquals(1, routes3.size)

    assertTrue(
      routes1[0].segments.all { s ->
        s.line in setOf(
          piccadillyLine,
          victoriaLine
        )
    ...
    assertTrue(routes1[1].segments.all { s -> s.line == districtLine })
  }

  @Test
  fun `can optimise for number of changes`() {

    val routes = map.routesFrom(
      southKensington,
      victoria,
      optimisingFor = Route::numChanges
    )
  ...
  fun `can optimise for duration`() {
    ...
    val routes =
      map.routesFrom(southKensington, victoria, optimisingFor = Route::duration)
    assertEquals(2, routes.size)
  ...
  fun findIn(route: Route, station: Station) =
